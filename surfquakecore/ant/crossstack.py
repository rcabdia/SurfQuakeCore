...
class noisestack:
    ...
    def __init__(self, output_files_path, stations, channels, stack, power, autocorr, min_distance,
        ...
        """
                ...
                :param cpu_count: number of parallel worker processes (default: os.cpu_count() - 1)
        """
        ...
        self.cpu_count = cpu_count if cpu_count is not None else max(1, os.cpu_count() - 1)
        self.check_path()
...
if __name__ == "__main__":
    ...
    # --- Paths ---
    parser.add_argument("input_path",   help="Directory containing the matrix pickle files (output of process_ant)")
    parser.add_argument("output_path",  help="Directory where stack files will be written")

    # --- Station / channel selection ---
    parser.add_argument("--stations",   nargs="*", default=[],
                        help="Station whitelist (e.g. --stations STA1 STA2). Empty = all stations.")
    parser.add_argument("--channels",   nargs="+", required=True,
                        help="Channel suffix(es) to process (e.g. --channels Z BHZ)")

    # --- Stacking ---
    parser.add_argument("--stack",      default="Linear",
                        choices=["Linear", "PWS", "nrooth"],
                        help="Stacking method (default: Linear)")
    parser.add_argument("--power",      type=float, default=2.0,
                        help="PWS or nrooth power (default: 2.0)")

    # --- Cross-correlation options ---
    parser.add_argument("--autocorr",   action="store_true", default=False,
                        help="Include autocorrelations (i==j pairs). Default: off.")
    parser.add_argument("--min-dist",   type=float, default=0.0,
                        help="Maximum inter-station distance in km to include (default: 0 = no limit). "
                             "Note: pairs with dist/1000 <= min_dist are kept.")

    # --- Daily stacks ---
    parser.add_argument("--daily-stacks", action="store_true", default=False,
                        help="Also save per-day stacks alongside the full stack.")
    parser.add_argument("--overlap",    type=float, default=75.0,
                        help="Overlap percentage for daily partial stacks (default: 75)")

    # --- Parallelism ---
    parser.add_argument("--workers",    type=int, default=None,
                        help="Number of parallel worker processes (default: os.cpu_count() - 1)")

    # --- Optional post-processing ---
    parser.add_argument("--rotate",     action="store_true", default=False,
                        help="Run rotate_horizontals() after stacking (requires EE/EN/NN/NE components).")
    parser.add_argument("--rotate-daily", action="store_true", default=False,
                        help="Run rotate_specific_daily() after stacking.")

    args = parser.parse_args()

    stacker = noisestack(
        output_files_path = args.input_path,   # input pickles live here; stack/ subdir created inside
    ...
    if args.rotate:
        print("Running horizontal rotation...")
        stacker.rotate_horizontals()

    if args.rotate_daily:
        print("Running daily horizontal rotation...")
        stacker.rotate_specific_daily()

    print("Done. Results written under:", args.output_path)
