...
import obspy
from surfquakecore.ant.signal_processing_tools import noise_processing, noise_processing_horizontals
...
class disp_maps_tools:
    ...
    @classmethod
    def disp_maps_availables(cls, disp_maps_path):
        """
        CLI version: disp_maps_path must be passed explicitly instead of
        reading from the ISP GUI constant DISP_MAPS.
        """
        file_to_add = []
        file_checked = []

        for top_dir, _, files in os.walk(disp_maps_path):
            for file in files:
